...
typedef enum {SHUTTLE, CYBORG, ELEPHANT} modelname;
...
public:
	int currenttime_ = 1;
	const int maxtime_ = 4000;
	const float fps = 30.0;
	vmath::vec3 planetrans_ = { 0.0f, -5.0f, 0.0f }; // the center of the green plane
	vec3 planeNormal = { 1, 1, 0 };
	Plane*  p = new Plane({ 0.0f, 0.0f, 0.0f }, planeNormal);
	Plane* p1 = new Plane(vec3(100, -5, 100), vec3(0, 1, 0));
	...
	void init() {
		delete object;
		delete f;
		if (currmod == SHUTTLE) m = "shuttle";
		else if (currmod == CYBORG) m = "cyborg";
		else m = "ele";
		object = new DeformableObject(gridsize, 3, m, r, tr);
		f = new Forces(object);
		currenttime_ = 1;
	}
	...
	void update() {
		if (animate) {
			float currTime = (float)currenttime_ / maxtime_, prevTime = 0;
			float delT = currTime - prevTime;
			f->updateForces();
			if(plane)
			object->collisionResolution(p, numpts);
			//object->checkCollision(p2);
			object->updateModel();
			object->collisionResolution(p1, numpts);
			//object->checkIntersection();
			object->updatePosition(delT);
			prevTime = currTime;
		}
	}
};
...
public:
	...
	std::vector<vmath::vec3> cnormals_; // cube normal
										...
										// the green plane
	vmath::vec3 planescale_ = vmath::vec3(200.0f, 0.02f, 200.0f);
...
public:
	...
	void update() {
		...
		if (trans[0] != lasttrans[0] || trans[1] != lasttrans[1] || trans[2] != lasttrans[2]) {
			...
			if (trans[1] != lasttrans[1]) {
				tt = -(trans[1] - lasttrans[1]) * v;
			}
			if (trans[2] != lasttrans[2]) {
				tt = -(trans[2] - lasttrans[2]) * w;
			}
			vmath::mat4 mt = vmath::translate(lasttrans - trans);
			M = mt * M;
...
void setmaterial(std::string whichone) {
	...
	else if (whichone == "redplane") {
		ambient = vmath::vec4(0.5, 0.0, 0.0, 1.0);
		diffuse = vmath::vec4(0.5, 0.0, 0.0, 1.0);
		specular = vmath::vec4(0.0, 0.0, 0.0, 1.0);
	}
	else {
...
void display() {
	...
	glPushMatrix();
	setmaterial("model");
	DeformableObject* obj = g_scene_.animate_.object;
	Model* mod = obj->model;
	for (int i = 0; i < mod->triangles.size(); i++) {
		vec3 p0 = mod->vertices[mod->triangles[i][0]].pos;
		vec3 p1 = mod->vertices[mod->triangles[i][1]].pos;
		vec3 p2 = mod->vertices[mod->triangles[i][2]].pos;
		//glPointSize(8);
	...
	if(trackobj)
		g_scene_.camera_.trans = obj->points[0][0][0].position + vec3(0, 1, 1);

	if (showCube) {
		int n = g_scene_.animate_.object->numOfPoints;
		glPushMatrix();
		setmaterial("plane");
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < n; j++) {
				for (int k = 0; k < n; k++) {
					vec3 p = g_scene_.animate_.object->points[i][j][k].position;
					glPointSize(8);
					glBegin(GL_POINTS);
	...
	if (plane) {
		glPushMatrix();
		setmaterial("redplane");
		glTranslatef(g_scene_.animate_.p->point[0], g_scene_.animate_.p->point[1], g_scene_.animate_.p->point[2]);
		glRotatef(-45, 0, 0, 1);
		//glRotatef(45, 1, 0, 0);
		glScalef(20, gmodel_.planescale_[1], 20);
		glBegin(GL_TRIANGLES);
		for (int i = 0; i < 2; ++i) {
			glNormal3f(gmodel_.pnormals_[3 * i], gmodel_.pnormals_[3 * i + 1], gmodel_.pnormals_[3 * i + 2]);
			for (int j = 0; j < 3; ++j) {
...
void init_gui() {
	...
	TwAddVarRW(operbar, "Animate", TW_TYPE_BOOL8, &g_scene_.animate_.animate, " help='start/stop animation'");
	TwAddVarRW(operbar, "Show box", TW_TYPE_BOOL8, &showCube, " help=''");
	TwAddVarRW(operbar, "Track Object", TW_TYPE_BOOL8, &trackobj, " help=''");
	TwType modeltype = TwDefineEnum("modeltype", NULL, 0);
	TwAddVarRW(operbar, "Model", modeltype, &currmod, " enum='0{Shuttle},1{Cyborg},2{Elephant}' ");
	TwAddVarRW(operbar, "Grid size", TW_TYPE_INT16, &gridsize, " help=''");
	TwAddVarRW(operbar, "Collision Points", TW_TYPE_INT16, &numpts, " help=''");
	TwAddVarRW(operbar, "Initial Rotation", TW_TYPE_BOOL8, &r, " help=''");
	TwAddVarRW(operbar, "Second Plane", TW_TYPE_BOOL8, &plane, " help='include plane'");
	TwAddSeparator(operbar, "", "");
...
GLvoid Timer(int value) {
	...
	if (value)
		glutPostRedisplay();
	glutTimerFunc(g_scene_.animate_.maxtime_ / g_scene_.animate_.fps, Timer, value);
}
